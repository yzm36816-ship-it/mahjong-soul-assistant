...
type Metrics = { mode: number; promoted: boolean; reason: string; sources?: Record<string, number>; samples?: Record<string, number>; games?: Record<string, number>; dataset?: { generator?: string; generator_version?: string; sha256?: string; policy?: string; data_kind?: string }; tenpai?: { model: { brier: number; ece: number; precision: number; recall: number }; rule_bucket_baseline: { brier: number }; non_riichi_examples?: number; non_riichi_model?: { brier: number }; non_riichi_rule_baseline?: { brier: number } }; waits?: { model_recall_at_5: number | null; prior_recall_at_5: number | null; examples: number }; ron?: { model_brier: number | null; prior_brier: number | null; examples: number } }
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <div className="page-content">
        ...
        <section id="models" className="section"><div className="section-heading"><div><div className="overline">02 / MODEL LAB</div><h2>模型实验</h2><p>训练、验证和测试按整场牌局分开。合成数据的分数只证明实验管线能运行。</p></div><span className="heading-note">无达标报告则不显示实时概率</span></div><div className="model-grid">{[3, 4].map(mode => { const report = metrics[String(mode)]; return <div className="card model-card" key={mode}><div className="model-title"><span className="mode-mark">{mode}P</span><div><h3>{mode === 3 ? '三人麻将' : '四人麻将'}</h3><small>{report?.sources?.synthetic_demo ? (report?.dataset?.generator === 'RiichiEnv' ? 'RiichiEnv 模拟对局' : '合成数据实验') : '等待真实数据'}</small></div><span className={`badge ${report?.promoted ? 'good' : ''}`}>{report?.promoted ? '已验证' : '实验中'}</span></div><div className="model-stats"><div><span>测试牌局</span><strong>{report?.games?.test ?? '—'}</strong></div><div><span>听牌 Brier ↓</span><strong>{decimal(report?.tenpai?.model.brier)}</strong></div><div><span>等待召回@5 ↑</span><strong>{percentage(report?.waits?.model_recall_at_5)}</strong></div></div><div className="comparison"><span>听牌误差与规则基线</span><div className="bar-row"><small>模型</small><div><i style={{width: `${Math.min(100, (report?.tenpai?.model.brier ?? 0) * 300)}%`}}/></div><b>{decimal(report?.tenpai?.model.brier)}</b></div><div className="bar-row baseline"><small>规则</small><div><i style={{width: `${Math.min(100, (report?.tenpai?.rule_bucket_baseline.brier ?? 0) * 300)}%`}}/></div><b>{decimal(report?.tenpai?.rule_bucket_baseline.brier)}</b></div></div><div className="model-foot">{report?.reason ?? '尚无训练报告'}。未立直听牌 Brier（模型 / 规则）：{decimal(report?.tenpai?.non_riichi_model?.brier)} / {decimal(report?.tenpai?.non_riichi_rule_baseline?.brier)}，样本 {report?.tenpai?.non_riichi_examples ?? '—'}；等待样本：{report?.waits?.examples ?? '—'}；荣和风险 Brier：{decimal(report?.ron?.model_brier)}；全部测试样本：{report?.samples?.test ?? '—'}。{report?.dataset?.generator && <><br/>数据：{report.dataset.generator} {report.dataset.generator_version} · SHA256 {report.dataset.sha256?.slice(0, 12)}…</>}</div></div> })}</div></section>
