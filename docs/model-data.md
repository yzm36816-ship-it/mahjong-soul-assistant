...
`tools/generate_simulations.py` 用 RiichiEnv 0.4.10 的 `GameRule.default_mjsoul()` 预设生成三麻、四麻东风战。策略优先和牌，以固定概率立直，其他时候选向听数最低的舍牌并跳过副露。策略随机数使用固定 seed；精确数据重建依靠保存的完整事件日志。生成器只接受无副露日志，遇到不支持的事件直接拒绝，不会编造手牌。完整模拟日志提供各家暗手，但转换器只把暗手放进标签，训练特征仍由公开状态和自己手牌构成。

输出位于私有 `data/simulations/`，公开仓库另附仅含模拟对局的压缩 MJAI 事件日志与去路径清单。清单记录模拟器版本、规则、策略、分局数量、样本数量和 SHA256。RiichiEnv 0.4.10 传入相同 `reset(seed)` 时，东风战后续各局牌墙仍可能不同；因此精确重建以事件日志为准，`tools/rebuild_simulations.py` 校验重建后的数据摘要。训练时校验清单摘要，测试集按整场东风战隔离。报告额外列出**未立直状态**的听牌指标，避免立直标志让整体指标虚高。模拟数据沿用 `synthetic_demo` 来源，永远不会满足实时概率的晋级门槛；真人对局需要另行评估视觉识牌与预测效果。
